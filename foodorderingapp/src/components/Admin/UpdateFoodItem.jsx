...
import { updateFoodItem } from '../../services/api';
...
const UpdateFoodItem = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await updateFoodItem(foodItemData.categoryId, foodItemData.foodItemId, foodItemData);
  ...
  return (
    <div className="min-h-screen bg-gray-800 text-white flex flex-col items-center justify-center px-4 py-8">
      <div className="w-full max-w-2xl bg-gray-900 rounded-lg shadow-lg p-6">
        <h2 className="text-4xl font-bold text-orange-500 mb-8 text-center">Update FoodItem Details</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center bg-gray-700 p-4 rounded-lg">
            ...
            <div className="flex-grow">
              <label className="text-orange-400 mb-2">FoodItem ID</label>
              <input
                type="number"
                className="w-full px-4 py-2 rounded-md bg-gray-800 text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-orange-500"
                value={foodItemData.foodItemId}
                onChange={(e) => setFoodItemData({ ...foodItemData, foodItemId: e.target.value })}
                required
              />
            </div>
          </div>
